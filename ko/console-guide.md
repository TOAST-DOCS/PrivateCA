...
Private CA에서 인증서를 발급받기까지의 과정은 다음과 같습니다.
...
4. **인증서 발급**: 인증서 템플릿을 통해 실제 사용할 인증서를 발급받습니다.
...
!!! tip "알아두기"
    CRL과 OCSP는 인증서의 폐기 상태를 확인하는 방법입니다. CRL은 폐기된 인증서 목록을 제공하며, OCSP는 인증서 상태를 조회할 수 있습니다.
...
발급자 목록에서 발급자 이름을 클릭하면 상세 정보 페이지로 이동합니다. 상세 페이지에는 다음 정보가 표시되며, 상단의 다운로드 버튼을 통해서 인증서 PEM 파일을 다운로드 할 수 있습니다.
...
#### 인증서 템플릿 삭제
1. 인증서 템플릿 목록에서 삭제할 인증서 템플릿의 **[삭제]** 버튼을 클릭하거나, 상세 페이지에서 **[삭제]** 버튼을 클릭합니다.
...
1. 인증서 템플릿 상세 페이지 상단의 **[+ 인증서 신규 생성]** 버튼을 클릭합니다.
...
- **상태**: 인증서의 현재 상태
  ...
  - **revoked**: 폐기된 상태 (빨간색)
- **시리얼 번호**: 인증서의 고유 시리얼 번호
- **유효 시작 시각**: 인증서가 유효하게 된 시점 (유효 시작 시각)

각 인증서 항목에는 **[다운로드]** 및 **[폐기]** 버튼이 있어 인증서를 관리할 수 있습니다.

### 인증서 상세 정보

인증서 목록에서 공통 이름을 클릭하면 상세 정보 페이지로 이동합니다. 상세 페이지에는 다음 정보가 표시되며, 상단의 다운로드 버튼을 통해서 인증서 PEM 파일을 다운로드 할 수 있습니다.
...
1. 인증서 목록에서 폐기할 인증서 템플릿의 **[폐기]** 버튼을 클릭하거나, 상세 페이지에서 **[폐기]** 버튼을 클릭합니다.
2. 확인 대화상자에서 <strong>[폐기]</strong>를 클릭하여 폐기를 확정합니다.
...
- **OCSP (Online Certificate Status Protocol)**: 저장소의 OCSP URL을 통해 인증서 상태를 조회할 수 있습니다.
...
!!! danger "주의"
    HMAC Key는 토큰 생성 시 한 번만 표시됩니다. 반드시 안전한 곳에 복사하여 보관해야 하며, 이후에는 다시 확인할 수 없습니다. HMAC Key를 분실한 경우 새로운 토큰을 생성해야 합니다.
