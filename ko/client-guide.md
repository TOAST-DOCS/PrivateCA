...
# ACME 인증서 갱신 가이드(Certbot, acme.sh)
...
!!! tip "알아두기"
    ...
    - **acme.sh**: 순수 Unix Shell로 작성된 경량 ACME 클라이언트로, 인증서를 자동으로 발급하고 갱신합니다.
...
### 1. Base 인증서 발급 { #issue-a-base-certificate }
...
### 2. ACME 서버 정보 확인 { #verify-acme-server-information }
...
## 인증서 갱신하기 { #renew-a-certificate }

ACME 클라이언트로 Certbot 또는 acme.sh를 사용할 수 있습니다. 사용 환경에 맞는 도구를 선택하여 진행하세요.

- **Certbot**: Let's Encrypt에서 개발한 널리 사용되는 ACME 클라이언트로, Python 기반입니다.
- **acme.sh**: 순수 Shell 스크립트로 작성된 경량 ACME 클라이언트로, 의존성이 적고 설치가 간편합니다.

!!! tip "알아두기"
    Kubernetes 환경에서 인증서를 자동으로 관리하려면 [ACME 인증서 갱신 가이드(cert-manager)](cert-manager-guide.md)를 참고하세요.

<a id="renew-your-certificate-with-certbot"></a>
### Certbot을 이용한 인증서 갱신 { #renew-your-certificate-with-certbot }
...
| `--manual-auth-hook` | Challenge 검증 전에 실행할 스크립트를 지정합니다. 빈 스크립트라도 갱신 시 오류 방지를 위해 필요합니다. | O | - |
...
| `--key-type` | 개인 키 유형(`rsa`, `ecdsa`)을 지정합니다. | X | rsa |
...
!!! tip "알아두기"
    ...
    - `manual-auth-hook`은 인증서 갱신 시 안정성 확보를 위해 반드시 필요합니다.
...
### acme.sh를 이용한 인증서 갱신 { #certificate-renewal-with-acmesh }

acme.sh는 순수 Unix Shell로 작성된 경량 ACME 클라이언트입니다. 의존성이 적고 설치가 간편하여 다양한 환경에서 사용할 수 있습니다.
...
!!! tip "알아두기"
    - 컨테이너나 새로운 쉘 세션에 재접속 시 `. ~/.acme.sh/acme.sh.env` 명령어로 환경 변수를 다시 로드해야 합니다.
    - 설치 시 이메일 주소는 선택 사항입니다.
...
!!! danger "주의"
    - 계정 등록은 **최초 1회만** 수행하면 됩니다.
    - 등록 후에는 인증서 발급 시 `--eab-kid`와 `--eab-hmac-key` 옵션을 생략할 수 있습니다.
...
| 옵션 | 설명 | 필수 | 기본값 |
|------|------|------|--------|
| `--register-account` | ACME 서버에 계정을 등록합니다. 최초 1회만 실행합니다. | O(최초) | - |
| `--issue` | 인증서를 발급합니다. | O | - |
| `--server` | ACME 서버의 Directory URL을 지정합니다. | O | - |
| `--eab-kid` | External Account Binding의 키 ID입니다. Private CA에서 발급한 ACME 토큰 ID를 입력합니다. 계정 등록 시에만 필요합니다. | O(등록 시) | - |
| `--eab-hmac-key` | EAB HMAC 키(Base64 인코딩)입니다. Private CA에서 발급한 ACME 토큰 HMAC 키를 입력합니다. 계정 등록 시에만 필요합니다. | O(등록 시) | - |
| `-d` | 인증서에 포함할 도메인을 지정합니다. Base 인증서의 CN과 SAN을 콘솔에서 확인하여 정확히 입력해야 합니다. 여러 개 지정 가능합니다. | O | - |
| `--standalone` | Standalone 모드로 HTTP-01 Challenge를 처리합니다. acme.sh가 임시 웹 서버를 실행합니다. | O | - |
| `--httpport` | HTTP-01 Challenge에 사용할 포트 번호를 지정합니다. | X | 80 |
| `--keylength` | 개인 키 길이를 지정합니다. RSA는 `2048`, `3072`, `4096`, ECDSA는 `ec-256`, `ec-384`, `ec-521` 값을 사용할 수 있습니다. | X | 2048 |
| `--debug` | 디버그 모드로 실행하여 상세 로그를 출력합니다. | X | - |

!!! tip "알아두기"
    - `--standalone` 모드는 acme.sh가 임시 웹 서버를 실행하여 Challenge를 처리합니다. 80번 포트를 사용할 수 있어야 합니다.
    - 계정 등록 후에는 `--eab-kid`와 `--eab-hmac-key` 옵션 없이 인증서를 발급할 수 있습니다.
...
!!! danger "주의"
    - 80번 포트가 열려 있고 사용 가능해야 합니다.
    - 기존 웹 서버가 80번 포트를 사용 중이면 일시적으로 중단해야 합니다.
    - 다른 포트를 사용하려면 `--httpport` 옵션을 추가합니다.
...
!!! tip "알아두기"
    - `<도메인명>.cer`: 리프 인증서만 포함
    - `fullchain.cer`: 리프 인증서 + 중간 인증서 + 루트 인증서 전체 체인
    - `ca.cer`: CA 체인(중간 인증서 + 루트 인증서)
...
!!! tip "알아두기"
    - `--key-file`: 개인 키 파일 저장 경로
    - `--cert-file`: 리프 인증서 저장 경로
    - `--fullchain-file`: 전체 체인(리프 + CA 체인) 저장 경로
    - `--ca-file`: CA 체인 저장 경로
    - `--reloadcmd`: 인증서 설치 후 자동으로 실행할 명령어

<a id="certificate-renewal-with-acmesh-set-up-certificate-auto-renewal"></a>
#### 인증서 자동 갱신 설정하기

acme.sh는 설치 시 자동으로 cron 작업을 등록하여 인증서 만료를 확인하고 갱신합니다.

##### 갱신 전제 조건

- 최초 발급 이력이 있어야 합니다.
- `~/.acme.sh/<도메인>/` 디렉터리에 인증서 파일들이 있어야 합니다.

##### 자동 갱신 설정

acme.sh 설치 시 자동으로 cron 작업이 등록됩니다.
...
!!! danger "주의"
    - ED25519 키 타입은 현재 acme.sh에서 지원하지 않을 수 있습니다. 인증서 체인에 ED25519 알고리즘을 사용하는 인증서가 포함된 경우 발급 및 갱신이 불가능합니다.
    - 인증서 체인은 최소 3단 이상(Root → Intermediate → Leaf) 구성되어야 합니다.
    - `Le_RenewalDays` 값은 ACME 서버의 Rate Limit 정책을 고려하여 신중히 조정해야 합니다.
    - Standalone 모드를 사용하는 경우, 갱신 시 80번 포트가 사용 가능해야 하므로 웹 서버를 일시 중단하는 스크립트가 필요할 수 있습니다.
...
1. **ACME Directory URL 확인**: `--server` 옵션의 URL이 올바른지 확인합니다.
2. **EAB 인증 정보 확인**: 계정 등록이 완료되었는지 확인합니다. 등록되지 않았다면 `--register-account` 명령어로 먼저 계정을 등록합니다.
3. **도메인 검증 실패**: 80번 포트가 열려 있고 사용 가능한지 확인합니다. 기존 웹 서버가 80번 포트를 사용 중이면 일시적으로 중단해야 합니다.
4. **디버그 모드**: `--debug` 옵션을 추가하여 상세 로그를 확인합니다.
...
1. **인증서 정보 확인**: 최초 발급 시 사용한 설정이 유지되고 있는지 확인합니다.
2. **Cron 작업 확인**: Cron 작업이 정상적으로 등록되어 있는지 확인합니다.
3. **로그 확인**: `~/.acme.sh/<도메인>/<도메인>.log` 파일에서 오류 메시지를 확인합니다.
4. **수동 갱신 테스트**: `acme.sh --renew -d example.com --force --debug` 명령어로 수동 갱신을 시도하여 문제를 진단합니다.

<a id="about-acme-protocol"></a>
## ACME 프로토콜 정보 { #about-acme-protocol }
...
- [acme.sh 공식 문서](https://github.com/acmesh-official/acme.sh)
- [acme.sh DNS API 목록](https://github.com/acmesh-official/acme.sh/wiki/dnsapi)
- [ACME 프로토콜 명세(RFC 8555)](https://datatracker.ietf.org/doc/html/rfc8555)
- [Let's Encrypt - Challenge Types](https://letsencrypt.org/docs/challenge-types/)
- [ACME 인증서 갱신 가이드(cert-manager)](cert-manager-guide.md)
